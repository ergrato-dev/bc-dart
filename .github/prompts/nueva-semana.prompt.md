...
name: "Nueva semana"
argument-hint: "Número de semana (ej: 05), nombre del tema (ej: genericos_y_pattern_matching), objetivos de aprendizaje y conceptos clave que se enseñan"
mode: "agent"
---

# Scaffold de nueva semana — Bootcamp Dart
...
> Semana 10 (proyecto final) es la única excepción sin `1-teoria/`/`2-practicas/`/`0-assets/`/`4-recursos/`.

## Convenciones obligatorias

- **Idioma documentación**: español
- **Idioma código**: inglés (variables, funciones, tipos, nombres de archivo)
- **Dart**: null safety estricto, `const`/`final` siempre que se pueda, sin `dynamic`
- **Sin Flutter**: ningún import de `package:flutter/...` en ningún archivo de este repo
- **dart pub**: SOLO este, versiones exactas para paquetes core del curso
- **Docker**: `3-proyecto/starter/` incluye `Dockerfile` + `docker-compose.yml` (imagen `dart:stable`)
...
5. Crear `2-practicas/ejercicio-01/` con README y `starter/bin/main.dart` usando patrón uncomment
6. Crear `3-proyecto/README.md` con instrucciones genéricas adaptables a dominios
7. Crear `3-proyecto/starter/` con estructura Dart básica, `Dockerfile`/`docker-compose.yml` y TODOs genéricos
